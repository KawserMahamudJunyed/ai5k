...
function OrgOverviewInner({ orgId }: { orgId: string }) {
    ...
    return (
      <main className="flex min-h-[60vh] items-center justify-center p-6">
        <div className="max-w-md text-center border border-hairline rounded-md p-10 bg-canvas">
          <h1 className="text-card-heading font-display text-ink mb-3">Private organization</h1>
  ...
  if (loadError === "not-found") {
    return (
      <main className="flex min-h-[60vh] items-center justify-center p-6">
        <div className="max-w-md text-center border border-hairline rounded-md p-10 bg-canvas">
          <h1 className="text-card-heading font-display text-ink mb-3">Organization not found</h1>
          <Link href="/organizations" className="text-blue underline underline-offset-4">← Your organizations</Link>
        </div>
      </main>
    );
    ...
    <main className="max-w-text mx-auto px-6 py-12">
      <header className="mb-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <h1 className="text-section-heading font-display text-ink">{org.name}</h1>
          {isAdmin && (
      ...
      {editing ? (
        <form onSubmit={handleSave} className="bg-canvas border border-hairline rounded-md p-8 space-y-5">
        ...
        <div className="border-y border-hairline divide-y divide-hairline">
          <div className="py-4"><p className="text-sm text-muted">Description</p>
            <p className="text-ink mt-1">{org.description || <span className="text-muted-2 italic">None yet.</span>}</p></div>
          <div className="py-4"><p className="text-sm text-muted">Website</p>
            ...
            ) : <p className="text-muted-2 italic mt-1">None yet.</p>}</div>
          ...
          <div className="py-4"><p className="text-sm text-muted">Created</p>
            <p className="text-ink mt-1">{new Date(org.created_at).toLocaleDateString()}</p></div>
        </div>
      )}
    </main>
  );
