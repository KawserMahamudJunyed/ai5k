...
function PublicProfileInner({ profileId }: { profileId: string }) {
    ...
    return (
      <main className="flex min-h-[60vh] items-center justify-center p-6">
        <div className="max-w-md text-center border border-white/10 rounded-md p-10 bg-void">
          <h1 className="text-card-heading font-display text-white mb-3">Private profile</h1>
  ...
  if (loadError === "not-found") {
    return (
      <main className="flex min-h-[60vh] items-center justify-center p-6">
        <div className="max-w-md text-center border border-white/10 rounded-md p-10 bg-void">
          <h1 className="text-card-heading font-display text-white mb-3">Profile not found</h1>
  ...
  return (
    <main className="max-w-text mx-auto px-6 py-12">
      <header className="border-b border-white/10 pb-8 mb-8">
        ...
        <h1 className="text-section-heading font-display text-white">{profile.display_name}</h1>
        {profile.headline && <p className="text-body-lg text-white mt-3">{profile.headline}</p>}
      ...
      {profile.portfolio_links.length > 0 && (
        <section className="mb-10">
          <h2 className="text-feature-heading font-display text-white mb-4">Portfolio</h2>
          <div className="divide-y divide-hairline border-y border-white/10">
            {profile.portfolio_links.map((l, i) => (
              <a key={i} href={l.url} target="_blank" rel="noopener noreferrer"
                className="flex items-center justify-between py-3 hover:bg-navy/40 transition-colors">
      ...
      {claims.length > 0 && (
        <section className="mb-10">
          <h2 className="text-feature-heading font-display text-white mb-4">Skills</h2>
          <div className="flex flex-wrap gap-2">
            {claims.map((c) => (
              <span key={c.id} className="inline-flex items-center gap-2 border border-white/10 rounded-full px-3 py-1.5 text-sm">
                {c.skill_name}
