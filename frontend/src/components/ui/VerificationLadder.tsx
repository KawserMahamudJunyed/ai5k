  ...
  return (
    ...
    >
      <motion.div 
        ...
        className="flex items-end gap-2 md:gap-4 min-w-[840px] h-[300px]"
      >
        {tiers.map((tier, i) => {
          const isTopTier = tier.weight === 1.0;
          const isActive = activeTier !== undefined && activeTier === i;
          
          // Map weight to pixel height (min 40px, max 220px)
          const barHeight = Math.max(40, Math.floor(tier.weight * 220));
          ...
          return (
            ...
            >
              <motion.div 
                ...
                className="mb-4"
              >
                <p className={`text-[10px] md:text-xs font-medium mb-1 ${isTopTier ? 'text-brand-mint' : 'text-white'} leading-tight`}>
                  {tier.name}
                </p>
                <p className="font-mono text-[9px] md:text-[10px] text-fog uppercase tracking-widest">
              ...
              <motion.div 
                ...
                role="img"
                aria-label={`Tier ${i}: ${tier.name}, weight ${tier.weight.toFixed(2)}`}
                title={tier.name}
              >
